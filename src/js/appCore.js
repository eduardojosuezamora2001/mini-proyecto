const AppCore = (() => {
  ...
  function page(items, current = 1, size = PAGE_SIZE) { const totalPages = Math.max(1, Math.ceil(items.length / size)); const safePage = Math.min(Math.max(1, current), totalPages); return { items: items.slice((safePage - 1) * size, safePage * size), current: safePage, totalPages, totalItems: items.length }; }
  ...
  return { PAGE_SIZE, session, user, isAdmin, can, applyPermissions, debounce, page, pagination, notify };
})();
