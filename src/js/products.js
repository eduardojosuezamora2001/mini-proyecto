const PRODUCT_KEY = "adminpro_products";
...
function renderProducts() {
  ...
  tableBody.innerHTML = filtered.length ? filtered.map(buildRow).join("") : `<tr><td colspan="6" style="padding:32px;text-align:center">No se encontraron productos.</td></tr>`;
...
function buildRow(product) {
  ...
  return `<tr class="${low ? "row-warning" : ""}" data-id="${escapeHtml(product.id)}">
    ...
    <td>${escapeHtml(product.category)}</td><td class="align-right">$${Number(product.price).toFixed(2)}</td>
    ...
    <td><div class="product-crud"><button data-action="edit">Editar</button><button data-action="delete">Eliminar</button></div></td></tr>`;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  tableBody.addEventListener("click", async (event) => {
    ...
    if (button.dataset.action === "edit") { const changes = await askProduct(item); if (changes) Storage.update(PRODUCT_KEY, id, changes); }
    else if (confirm(`¿Eliminar ${item.name}?`)) Storage.delete(PRODUCT_KEY, id);
    renderProducts();
  });
